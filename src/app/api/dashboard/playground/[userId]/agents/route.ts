...
type PatchRequestBody = Partial<IAgent> & { agentId: string };
...
export async function POST(req: NextRequest, context: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const { name, provider, modelName, avatar, avatarAssetUid, color, welcomeMessage, placeholderText } = body;
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ userId: string }> }) {
  try {
    ...
    if (!agentToUpdate || !agentToUpdate.contentstackUid) {
      return NextResponse.json({ error: "Agent not found or not synced with Contentstack." }, { status: 404 });
    }
    ...
    const csResponse = await fetch(
      `${apiHost}/v3/content_types/${CONTENTSTACK_AGENT_CT_UID}/entries/${agentToUpdate.contentstackUid}`,
      {
        method: "PUT",
        headers: {
          api_key: CONTENTSTACK_API_KEY!,
    ...
    if (!updatedAgent) {
      return NextResponse.json({ error: "Agent not found in MongoDB after update." }, { status: 404 });
    }
  ...
  try {
    ...
    if (agentToDelete.contentstackUid) {
      ...
      await fetch(`${apiHost}/v3/content_types/${CONTENTSTACK_AGENT_CT_UID}/entries/${agentToDelete.contentstackUid}`, {
        method: "DELETE",
        headers: { api_key: CONTENTSTACK_API_KEY!, authorization: CONTENTSTACK_MANAGEMENT_TOKEN! }
      });
    }
